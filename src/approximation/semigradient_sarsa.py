    ...
    for step in range(iterations):
        ...
        while not grid.game_over():
            ...
            if grid.is_terminal(state_t1):
                ...
                action_t1 = random_action(
                    max_dict(getQs(model, state_t1))[0], epsilon)
                model.theta += learning_rate * (
                    (reward_t + discount * model.predict(state_t1, action_t1))
...
def sarsa_example():
    ...
    values, policy, deltas = gradient_sarsa(grid)
    plt.plot(deltas)
    plt.show()
    print('Values:')
